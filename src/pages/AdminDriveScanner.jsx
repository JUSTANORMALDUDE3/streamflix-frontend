...
const generateThumbnailFromFileId = async (fileId) => {
    try {
        // Fetch only the first 8 MB (enough to seek and grab a frame)
        ...
        return new Promise((resolve) => {
            ...
            video.onloadedmetadata = () => {
                video.currentTime = Math.min(video.duration * 0.1, 10); // seek to 10% or 10s
            };
            video.onseeked = () => {
...
const AdminDriveScanner = () => {
    ...
    const handleSelect = async (file) => {
        ...
        const dataUrl = await generateThumbnailFromFileId(file.fileId);
    ...
    const handleRegenerate = async () => {
        if (!selected) return;
        setThumbLoading(true);
        const dataUrl = await generateThumbnailFromFileId(selected.fileId);
        setGeneratedThumbnail(dataUrl);
        setThumbLoading(false);
    };
